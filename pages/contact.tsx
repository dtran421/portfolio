...
const IdLabel = ({ mdScreen, heading, body }: IdLabelProps) =>
    mdScreen ? (
        <p className="space-x-1">
            <span className="font-medium text-zinc-700 dark:text-zinc-300 dark-transition">
                {heading}
            </span>
            :{" "}
            <span className="bg-zinc-400/30 dark:bg-zinc-600/30 dark-transition lg:text-lg rounded-lg px-2 py-1">
                {body}
            </span>
        </p>
    ) : (
        <div className="h-full flex flex-col justify-between items-center space-y-1">
            <p className="w-full h-1/3 flex justify-center items-center text-center font-medium text-zinc-700 dark:text-zinc-300 dark-transition">
                {heading}
            </p>
            <p className="w-full h-2/3 flex justify-center items-center text-sm lg:text-md text-center bg-zinc-400/30 dark:bg-zinc-600/30 dark-transition rounded-lg px-2 py-1">
                {body}
            </p>
        </div>
    );
                        ...
                        <div className="max-w-lg lg:w-full h-1/2 flex bg-zinc-200/75 dark:bg-zinc-800/75 dark-transition rounded-xl shadow-xl space-x-4 md:space-x-8 p-4 mx-4 md:mx-auto lg:mx-4">
                            <div className="flex justify-center items-center">
                                <div className="flex justify-center items-center w-24 h-24 md:w-32 md:h-32 bg-gradient-to-tr from-primary to-secondary rounded-full">
                                    <div className="font-Oxygen text-white text-3xl md:text-5xl font-bold">
                                        DT
                                    </div>
                                </div>
                            </div>
