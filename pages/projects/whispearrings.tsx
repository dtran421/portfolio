import { FC, useRef, useState, useReducer, useEffect, createContext } from "react";
...
const MenuBar = dynamic(() => import("../../components/Projects/Whispearrings/MenuBar"), { ssr: false });

const paragraphs = whispearringsData.data;

export const WhispearringsContext = createContext(null);
...
const Whispearrings: FC<{}> = () => {
   ...
   const assignPRef = (pRefs, { newPRef, el }) => {
      const newPRefs = pRefs;
      pRefs[newPRef].current = el;
      return newPRefs;
   };
   ...
   const [imgClass, setImgClass] = useState("z-10 relative rounded-xl opacity-0 transition duration-200 ease-linear");
   const [autoScroll, setAutoScroll] = useState(false);
   useEffect(() => {
      const scrollListener = () => {
         if (!autoScroll && activeP != -1 && pRefs[activeP].current) {
            let newActiveP;
            newActiveP = getActiveParagraph(pRefs);
   ...
   return (
      <ProjectLayout
         ...
         <div className="md:max-w-2xl lg:max-w-4xl xl:max-w-6xl relative flex flex-col items-center gap-y-20 mx-10 md:mx-auto mt-10 mb-20">
            ...
            <WhispearringsContext.Provider value={{ activeP, setActiveP, pRefs, setAutoScroll }}>
               {lgScreen && <MenuBar {...{ activeP, pRefs, setAutoScroll, paragraphs }} />}
            ...
            <div className="flex">
               <div className="w-full lg:w-1/2 flex flex-col items-center space-y-40 lg:space-y-80 py-10 lg:py-48">
                  {paragraphs.map((paragraph, idx) => {
                     return <Paragraph key={idx} {...{ lgScreen, setPRefs, idx, paragraph }} />;
                  })}
               </div>
               {lgScreen && (
                  <div className="w-1/2 relative flex flex-col space-y-32">
                     <div className="h-min sticky right-1/4 inset-y-1/4 flex justify-center items-start">
                        <div className="relative flex items-center bg-zinc-800 dark:bg-zinc-200 dark-transition rounded-xl p-2">
                           ...
                           <Image
                              alt={`whispearrings video ${activeP + 1}`}
                              src={`/img/whispearrings/whispearrings${activeP + 1}.gif`}
                              className={imgClass}
                              width={240}
                              height={520}
                           />
...
interface ParagraphProps {
   lgScreen: boolean;
   setPRefs: (value: { newPRef: number; el: HTMLParagraphElement }) => void;
   idx: number;
   paragraph: {
      heading: string;
      body: string;
...
const Paragraph: FC<ParagraphProps> = ({ lgScreen, setPRefs, idx, paragraph: { heading, body } }) => {
   return (
      <div id={`paragraph${idx + 1}`} className="space-y-6">
         <h2 className="text-3xl font-semibold">{heading}</h2>
         <p ref={(el) => setPRefs({ newPRef: idx, el })} className="text-xl leading-relaxed">
            {body}
         </p>
         {!lgScreen && (
