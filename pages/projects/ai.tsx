...
import { FiChevronDown, FiChevronRight, FiFile, FiFileText, FiFolder, FiImage } from "react-icons/fi";
...
const AI: FC<{}> = () => {
   ...
   const maxFiles = 14;
   let currIdx = 0;
   const [[foldersExpanded, numFiles], setFoldersExpanded] = useState([[], folders.length]);

   return (
      <ProjectLayout page="Machine Learning" accent="bg-ai" darkText={true}>
         <div className="md:max-w-2xl lg:max-w-4xl xl:max-w-6xl flex mx-10 md:mx-auto">
            <div className="w-full max-h-screen flex flex-col items-center bg-slate-300/50 dark:bg-slate-700/50 dark-transition backdrop-blur-lg rounded-xl p-6 lg:p-10 mt-20">
               <h1 className="w-full text-4xl lg:text-6xl font-bold">File Explorer</h1>
               <div className="w-full overflow-y-auto flex flex-col mt-10">
                  {folders.map((folder, idx, folders) => {
                     if (idx > 0) {
                        ...
                        if (foldersExpanded.includes(prevFolder.name)) {
                           ...
                           return (
                              <Folder
                                 ...
                                 isExpanded={foldersExpanded.includes(folder.name)}
                                 {...{ ...folder, foldersExpanded, numFiles, setFoldersExpanded }}
                              />
                           );
                        } else {
                           currIdx += 1;
                  ...
                  {_.times(maxFiles > numFiles ? maxFiles - numFiles : 0, (idx) => {
                     if (idx == 0) {
                        currIdx = numFiles;
                     } else {
                        currIdx += 1;
                     }
                     return <Folder key={idx} altColor={currIdx % 2 == 1} name={"_"} />;
                  })}
               </div>
            </div>
         </div>
      ...
      <div className="flex flex-col">
         <div
            className={`flex items-center text-lg lg:text-xl ${
               altColor ? "bg-slate-400/25 dark:bg-slate-600/25" : "bg-transparent"
               ...
               <>
                  <button
                     onClick={() =>
                        isExpanded
                           ? setFoldersExpanded([
                                foldersExpanded.filter((folder) => folder != name),
                                numFiles - contents.length
                             ])
                           : setFoldersExpanded([[...foldersExpanded, name], numFiles + contents.length])
                     }>
         ...
         {isExpanded && (
            <div className="flex flex-col">
               {contents.map((file, idx) => (
                  <File key={idx} idx={idx + (altColor ? 0 : 1)} text={file} src={`/files/ai/${name}/${file}`} />
               ))}
            </div>
         )}
      </div>
...
const File: FC<FileProps> = ({ idx, text, src }) => {
   let type;
   switch (src.split(".")[1]) {
      case "ipynb":
         type = "text";
